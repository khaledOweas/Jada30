...
import { JadaHeaderComponent } from "src/app/core/Components/jada-header/jada-header.component";
...
@Component({
  ...
  standalone: true,
  imports: [NgClass, KeeniconComponent, HeaderMenuComponent, NavbarComponent,NgIf, JadaHeaderComponent]
})
export class HeaderComponent implements OnInit, OnDestroy {
  ...
  updateProps(config: ILayout) {
    ...
    this.appHeaderDefaultContent = this.layout.getProp("app.header.default.content", config) as string;
    this.appHeaderDefaulMenuDisplay = this.layout.getProp("app.header.default.menu.display", config) as boolean;
    this.appPageTitleDisplay = this.layout.getProp("app.pageTitle.display", config) as boolean;

    // body attrs and container css classes
    this.appHeaderDefaultFixedDesktop = this.layout.getProp("app.header.default.fixed.desktop", config) as boolean;
    if (this.appHeaderDefaultFixedDesktop) {
      document.body.setAttribute("data-kt-app-header-fixed", "true");
    }

    this.appHeaderDefaultFixedMobile = this.layout.getProp("app.header.default.fixed.mobile", config) as boolean;
    if (this.appHeaderDefaultFixedMobile) {
      document.body.setAttribute("data-kt-app-header-fixed-mobile", "true");
    }

    this.appHeaderDefaultContainer = this.layout.getProp("appHeaderDefaultContainer", config) as "fixed" | "fluid";
    this.headerContainerCssClass = this.appHeaderDefaultContainer === "fixed" ? "container-xxl" : "container-fluid";

    this.appHeaderDefaultContainerClass = this.layout.getProp("app.header.default.containerClass", config) as string;
    if (this.appHeaderDefaultContainerClass) {
      this.headerContainerCssClass += ` ${this.appHeaderDefaultContainerClass}`;
    }

    this.appHeaderDefaultStacked = this.layout.getProp("app.header.default.stacked", config) as boolean;
