...
@Component({
  selector: "app-about-jada-events",
  standalone: true,
  imports: [ RouterLink, RouterLinkActive,  CommonModule,
    // JadaCardSystemServiceComponent, 
    CardModule, ButtonModule],
  templateUrl: "./about-jada-events.component.html",
  styleUrl: "./about-jada-events.component.scss",
  providers: [CoreService]
})
export class AboutJadaEventsComponent extends BaseComponent implements OnInit, OnDestroy  {
  ...
  constructor(private injector: Injector, private service: CoreService, private http: HttpClient,private PrimeNGConfig: PrimeNGConfig) {
    ...
    chunkBlogs(){
      this.visibleEvents=[];
      for(let i=this.currentIndex;i< this.events.length;i=i+2){
        this.visibleEvents.push(this.events.slice(i, i+2));
      }
    }
    ...
    next() {
      this.currentIndex = (this.currentIndex < this.visibleEvents.length - 1) ?
        this.currentIndex + 1 : 0;
    }
   
    prev() {
      this.currentIndex = (this.currentIndex > 0) ?
        this.currentIndex - 1 : this.visibleEvents.length - 1;
    }
